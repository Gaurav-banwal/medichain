...
Authenticates a user via Google. Validates the Google-issued ID token (`idToken`) on the backend. If the user does not exist in the database, a new record is created with the specified or default role (`CITIZEN`). On success, issues a signed session JWT in an `HttpOnly` cookie.
...
| Field | Type | Required | Description |
| :--- | :--- | :--- | :--- |
| `idToken` | `string` | **Yes** | Google OAuth ID Token (JWT format) |
| `role` | `string` | No | Target role for registration (default: `CITIZEN` if new user) |

### Request Example
```json
{
  "idToken": "eyJhbGciOiJSUzI1NiIsImtpZCI6IjFhMmIzYy...",
  "role": "CITIZEN"
}
```
...
### Error Responses
*   **`400 Bad Request`**: If the `idToken` is missing or is structurally invalid/expired.
*   **`500 Internal Server Error`**: For unexpected database or credential verification errors.
